...
const OrderItem: FC<OrderItemPropsI> = ({ main_image, title, _id, count, price }) => {
  return (
    <div className='bg-[#FBFBFB] h-[70px] w-full pr-2 md:pr-4 grid grid-cols-[4fr_1fr] sm:grid-cols-[3fr_1fr] gap-3'>
      ...
      <div className='w-full flex items-end justify-center sm:items-center sm:justify-between flex-col sm:flex-row gap-[6px] sm:gap-8'>
        ...
        <div className='flex items-center justify-between'>
          <h3 className='text-[18px] font-bold leading-4 text-green'>
            {formatPrice(Number(Number(count) * Number(price)))}
          </h3>
        </div>
      </div>
    </div>
