...
const useInteractionsInfoFeatures = () => {
  ...
  const socialPlatforms = [
    ...
    { key: '5', Component: TwitterShareButton, Icon: TwitterIcon, labelKey: 'twitter' },
    { key: '6', Component: FacebookShareButton, Icon: FacebookIcon, labelKey: 'facebook' },
  ];

  const items = socialPlatforms.map(({ key, Component, Icon, labelKey }) => ({
    key,
    label: (
      <Component url={SOCIAL_MEDIA_URL} title={post?.title || ''} className="flex items-center gap-2">
        <Icon size={20} round />
